/**
 * @file opt3.cpp
 * @brief MPI Poisson solver — Optimisation 3: remove redundant barrier and boundary re-stamp.
 *
 * Removes the MPI_Barrier and the two sets of per-iteration Dirichlet re-stamp
 * loops that were carried through opt1 and opt2. Profiling showed these
 * accounted for ~18\% of wall time at 8 ranks on the 64^3 test case:
 *   - MPI_Barrier:  added a global synchronisation on top of MPI_Allreduce
 *   - BC re-stamp:  O(N^2) work executed twice per iteration unnecessarily;
 *     the interior-only loop bounds already prevent any sweep from touching
 *     a Dirichlet node, so re-stamping is provably redundant.
 * The result is the minimal, clean iteration loop.
 *
