...
// ============================================================
// Halo exchange — NON-BLOCKING (Isend/Irecv), all 6 faces at once
//
// All 12 non-blocking operations are posted simultaneously before
// MPI_Waitall.  The network can now transfer all faces in parallel
...
int main(int argc,char*argv[]){
    ...
    do{
        ...
        // Re-stamp before sweep (carried over from opt1, not yet removed)
        if(bndXm) for(int j=1;j<=ly;++j) for(int k=1;k<=lz;++k)
        ...
        // Re-stamp after swap (carried over from opt1)
        if(bndXm) for(int j=1;j<=ly;++j) for(int k=1;k<=lz;++k)
            u[idx(1,j,k,LNY,LNZ)]=bcVal(gxs,gys+j-1,gzs+k-1);
        if(bndXp) for(int j=1;j<=ly;++j) for(int k=1;k<=lz;++k)
            u[idx(lx,j,k,LNY,LNZ)]=bcVal(gxs+lx-1,gys+j-1,gzs+k-1);
